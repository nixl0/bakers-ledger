    ...
    <div class="m-4 px-4">
        ...
        <div class="border shadow-xl rounded-md p-8 flex flex-row justify-center">
            <form action="/companies" method="POST" class="w-2/3 flex flex-col space-y-6">
                ...
                {{-- since --}}
                <x-input-box colname="год основания" colname_form="since" input_value="{{ old('since') }}" type="number" />
                @error('since')
                    <p class="text-red-500">
                        {{ $message }}
                    </p>
                ...
                {{-- email --}}
                <x-input-box colname="email" colname_form="email" input_value="{{ old('email') }}" type="email" />
                @error('email')
                    <p class="text-red-500">
                        {{ $message }}
                    </p>
