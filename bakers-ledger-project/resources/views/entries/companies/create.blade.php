    ...
    <div class="border shadow-xl rounded-md p-8 flex flex-row justify-center">
        <form action="/companies" method="POST" class="w-2/3 flex flex-col space-y-6">
            ...
            <div class="flex items-center space-x-2">
                ...
                <input list="legals" name="legal_id" class="w-full p-4 text-gray-900 border rounded-md" value="{{ old('legal_id') }}">
                <datalist id="legals">
            ...
            <div class="flex items-center space-x-2">
                ...
                <input type="number" name="since" class="w-full p-4 text-gray-900 border rounded-md" value="{{ old('since') }}">
            </div>
            @error('since')
                <p class="text-red-500">
                    {{ $message }}
            ...
            <div class="flex items-center space-x-2">
                ...
                <input type="email" name="email" class="w-full p-4 text-gray-900 border rounded-md" value="{{ old('email') }}">
            </div>
            @error('email')
                <p class="text-red-500">
                    {{ $message }}
            ...
            <div class="flex items-center space-x-2">
                ...
                <input list="users" name="user_id" class="w-full p-4 text-gray-900 border rounded-md" value="{{ old('user_id') }}">
                <datalist id="users">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" label="{{ $user->name }}, {{ $user->last_name }} {{ $user->first_name }}" />
                    @endforeach
                </datalist>
            </div>
            @error('district_id')
